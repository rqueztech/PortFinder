CREATE TABLE dns_records(
	...
	rfc VARCHAR(30),
	function VARCHAR(40),
	description VARCHAR(315)
);

INSERT INTO dns_records(type,type_id,rfc,function,description)
VALUES
('type','type_id','rfc','function','description'),
('A','1','RFC 1035［1］','Address record','Returns a 32ーbit IPv4 address、 most commonly used to map hostnames to an IP address of the host、 but it is also used for DNSBLs、 storing subnet masks in RFC 1101、 etc。'),
('AAAA','28','RFC 3596［2］','IPv6 address record','Returns a 128ーbit IPv6 address、 most commonly used to map hostnames to an IP address of the host。'),
...
('CDNSKEY','60','RFC 7344','Child Key','Child copy of DNSKEY record、 for transfer to parent'),
...
('CSYNC','62','RFC 7477','ChildーtoーParent Synchronization','Specify a synchronization mechanism between a child and a parent DNS zone。 Typical example is declaring the same NS records in the parent and the child zone'),
...
('EUI48','108','RFC 7043','MAC address （EUIー48）','A 48ーbit IEEE Extended Unique Identifier。'),
('EUI64','109','RFC 7043','MAC address （EUIー64）','A 64ーbit IEEE Extended Unique Identifier。'),
('HINFO','13','RFC 8482','Host Information','Providing MinimalーSized Responses to DNS Queries That Have QTYPE=ANY'),
('HIP','55','RFC 8005','Host Identity Protocol','Method of separating the endーpoint identifier and locator roles of IP addresses。'),
...
('KX','36','RFC 2230','Key Exchanger record','Used with some cryptographic systems （not including DNSSEC） to identify a key management agent for the associated domainーname。 Note that this has nothing to do with DNS Security。 It is Informational status、 rather than being on the IETF standardsーtrack。 It has always had limited deployment、 but is still in use。'),
...
('NAPTR','35','RFC 3403','Naming Authority Pointer','Allows regularーexpressionーbased rewriting of domain names which can then be used as URIs、 further domain names to lookups、 etc。'),
...
('OPENPGPKEY','61','RFC 7929','OpenPGP public key record','A DNSーbased Authentication of Named Entities （DANE） method for publishing and locating OpenPGP public keys in DNS for a specific email address using an OPENPGPKEY DNS resource record。'),
('PTR','12','RFC 1035［1］','PTR Resource Record ［de］','Pointer to a canonical name。 Unlike a CNAME、 DNS processing stops and just the name is returned。 The most common use is for implementing reverse DNS lookups、 but other uses include such things as DNSーSD。'),
...
('RRSIG','46','RFC 4034','DNSSEC signature','Signature for a DNSSECーsecured record set。 Uses the same format as the SIG record。'),
...
('SRV','33','RFC 2782','Service locator','Generalized service location record、 used for newer protocols instead of creating protocolーspecific records such as MX。'),
('SSHFP','44','RFC 4255','SSH Public Key Fingerprint','Resource record for publishing SSH public host key fingerprints in the DNS、 in order to aid in verifying the authenticity of the host。 RFC 6594 defines ECC SSH keys and SHAー256 hashes。 See the IANA SSHFP RR parameters registry for details。'),
...
('TXT','16','RFC 1035［1］','Text record','Originally for arbitrary humanーreadable text in a DNS record。 Since the early 1990s、 however、 this record more often carries machineーreadable data、 such as specified by RFC 1464、 opportunistic encryption、 Sender Policy Framework、 DKIM、 DMARC、 DNSーSD、 etc。'),
